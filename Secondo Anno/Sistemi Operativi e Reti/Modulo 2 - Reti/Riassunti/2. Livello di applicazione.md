...
- **Riga di stato**, presenta 3 campi: la versione del protocollo, un codice di stato e corrispettivo messaggio di stato.
    - _1xx Informational_, una risposta intermedia per comunicare lo stato di connessione o l'avanzamento della richiesta prima di completare l'azione richiesta e inviare una risposta finale.
    - _2xx Successful_, la richiesta è stata ricevuta con successo, compresa e accettata.  
...
L'idea alla base del caching è molto semplice. Una volta che un qualsiasi server impara la mappatura, la mette in cache, e restituisce immediatamente il 
mapping in caso di una query. Con il caching, il tempo di risposta ad una query è molto più veloce. Dopo un certo periodo di tempo (in genere dopo 2 giorni), le voci della cache vanno in timeout ovvero scompaiono. Inoltre i server TLD sono in genere memorizzati nella cache dei server DNS locali.
...
Al registrar dobbiamo fornire il nome e gli indirizzi IP dei DNS autoritativi primario e secondario. Il registrar inserisce poi due record nel TLD server .com: `(networkutopia.com, dns1.networkutopia.com, NS)`, `(dns1.networkutopia.com, 212.212.212.1, A)`.
...
Nell'architettura peer-to-peer ci sono coppie di host connessi in modo intermittente, chiamati peer, che comunicano direttamente l'uno con l'altro. I peer non appartengono ai fornitori dei servizi, ma sono computer fissi e portatili e smartphone controllati dagli utenti.
...
1. Distribuzione file client-server
    ...
    - Ogni client deve scaricare una copia del file. Sia $d_{min}$ la banda di download più bassa. Allora il tempo di download per il client con la banda minima è almeno $\frac{F}{d_{min}}$.
