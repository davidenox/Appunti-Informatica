...
- *Passive Optical Network*(**PON**): Usano splitter ottici non alimentati che trasmettono in broadcast verso gli utenti.
### FWA
...
Se il numero di pacchetti in arrivo è molto maggiore rispetto alla velocità di commutazione, si verifica il fenomeno di *accodamento* (**queuing**). C'è grande possibilità che i pacchetti accodati possano essere *scartati* (persi) se il buffer si riempie.
...
> Lo spettro di frequenza di collegamento viene diviso in *bande*, dedicate a ciascuna connessione. La larghezza di banda viene definita **ampiezza di banda** `bandwidth`.
...
I protocolli di rete sono organizzati in **livelli** o *strati*. Siamo interessati ai *servizi* offerti da un livello a quello superiore, si tratta del cosiddetto **modello di servizi** di un livello. Ogni livello fornisce il suo servizio effettuando determinate azioni all'interno del livello stesso ed utilizzando i servizi del livello immediatamente inferiore.
...
2. *Livello di Trasporto*:
	- Trasferisce i messaggi del livello di applicazione tra punti periferici gestiti dalle applicazioni. Si hanno due protocolli:
		...
		- `UDP`: Servizio non orientato alla connessione. Infatti, non è un servizio affidabile, e non garantisce la consegna dei dati. 
