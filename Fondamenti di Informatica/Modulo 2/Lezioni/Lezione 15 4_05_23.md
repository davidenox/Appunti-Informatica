...
Ma questo è equivalente a dire che, per ogni $x\in\{0,1\}^*$, x  L1c se e soltanto se f1(x)  L, 
ossia, per ogni x {0,1}*, x L1 se e soltanto se f1(x)  Lc 
ossia, L1 Lc
Poiché L1 è un qualsiasi linguaggio in coNP, questo dimostra che Lc è completo per coNP. 