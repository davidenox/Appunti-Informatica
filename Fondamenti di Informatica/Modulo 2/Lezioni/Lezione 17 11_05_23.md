...
Formalizzato il problema precedente mediante la tripla   , S,    : 
 = {  G = (V,E), u, v   : G è un grafo non orientato  un ciclo c in G che passa una e una sola volta attraverso ciascun nodo di G  u,v V}; 
S(G,u,v) = { p : p è un percorso in G }; 
(G,u,v, S(G,u,v)) = p  S(G,u,v)  che connette u a v. 
Data una qualunque codifica ragionevole per PHCc, per decidere se una parola x è contenuta in LPHCc(), dobbiamo verificare 
se x è la codifica di una istanza di PHCc, ossia, di un grafo che contiene un ciclo che attraversa tutti i nodi una e una sola volta, e di una coppia di suoi nodi, 
...
E, quindi, concludiamo (ad occhio) che PHCc è NP-completo. 